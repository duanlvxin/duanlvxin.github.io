...
2. 返回arr的最长无重复子数组的长度:https://leetcode.cn/problems/longest-substring-without-repeating-characters/description/
    两个指针，初始一个指针i指向第0个位置，一个指针j指向第1个位置；如果j指向的字符没有出现过，那么j往右边移动，更新长度，否则，i往右边移动
3. 盛水最多的容器: https://leetcode.cn/problems/container-with-most-water/description/
    两个指针，一个指向最左边，一个指向最右边；移动矮的那一边（因为如果移动高的，水量一定不会变多）
4. 三数之和：https://leetcode.cn/problems/3sum/description/
